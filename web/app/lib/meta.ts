...
import { SanityImage } from "~/post.server"
...
interface CustomMetaArgs<LoaderData> {
    ...
    imageSrc?: string | ((args: MetaFunctionArgs<LoaderData>) => string | SanityImage)
}
...
export const CustomMetaFunction = <LoaderData>(input: CustomMetaArgs<LoaderData>): MetaFunction => {
    return (args: MetaFunctionArgs<LoaderData>): HtmlMetaDescriptor => {
        ...
        let imageSrc: string | SanityImage = "image-977b4bac71559133b10faaac46c19118a61312d1-1920x500-png"
        if (input.imageSrc)
